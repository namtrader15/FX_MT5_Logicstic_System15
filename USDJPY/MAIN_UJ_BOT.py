...
from Entry_Super_UJ import get_final_trend_USDJPY  # Hàm lấy xu hướng
from TPO_POC_UJ import calculate_poc_value_USDJPY  # Hàm tính POC
from place_order_UJ import place_order_mt5  # Hàm thực hiện lệnh giao dịch
...
def get_position_info():
    positions = mt5.positions_get(symbol="USDJPY")  # Thay BTCUSD bằng USDJPY
    if positions:
    ...
    return None
def get_positions_pnl_USDJPY(symbol="USDJPY"):
    """
    Hàm lấy tổng PNL của tất cả các lệnh mở trên USDJPY.
    """
    positions = mt5.positions_get(symbol=symbol)  # Lấy danh sách vị thế cho USDJPY
    if positions:
...
def update_trade_status():
    ...
    pnl_USDJPY = get_positions_pnl_USDJPY("USDJPY")  # Lấy PNL của USDJPY
    ...
    trade_status["pnl"] = pnl_USDJPY if pnl_USDJPY is not None else "N/A"  # Gán PNL của USDJPY
...
# Hàm lấy xu hướng hiện tại từ hàm phân tích `get_final_trend_UC`
def get_trend():
    trend = get_final_trend_USDJPY()  # Không cần client Binance nữa
    print(f"Kết quả xu hướng USDJPY hiện tại: {trend}")
    return trend

# Kiểm tra giá POC và thực hiện lệnh
def check_poc_and_place_order(final_trend, symbol="USDJPY"):
    ...
    poc_value = calculate_poc_value_USDJPY()
    ...
    if price_difference_percent <= 0.03:
        ...
        print("[DEBUG] Không thực hiện lệnh vì chênh lệch vượt quá 0.03%.")
...
def close_position(position):
    ...
    """
    try:
        print("Bắt đầu xử lý đóng lệnh cho USDỤY...")
        positions = mt5.positions_get(symbol="USDJPY")
        if positions:
            for pos in positions:
                close_position(pos._asdict())
        else:
            print("Không có vị thế nào để đóng cho USDJPY.")
...
def buy_market():
    ...
    result = place_order_mt5(client, order_type="buy", symbol="USDJPY", risk_amount=RISK_AMOUNT)
...
def sell_market():
    client = mt5  # Tham số `client`, giả định nó là một phần của MetaTrader5
    result = place_order_mt5(client, order_type="sell", symbol="USDJPY", risk_amount=RISK_AMOUNT)
...
def status():
    update_trade_status()
    return jsonify({
        "current_price": get_realtime_price_mt5("USDJPY"),
        ...
        "pnl": trade_status["pnl"],  # PNL chỉ của USDJPY
        "tpo_poc_price": calculate_poc_value_USDJPY()  # Giá TPO/POC
    })
...
if __name__ == '__main__':
    if connect_mt5():
        print("Khởi động bot giao dịch USDJPY trên server Flask.")
        # Đóng lệnh cho USDCAD
     #   print("Bắt đầu xử lý đóng lệnh cho USDJPY...")
    #  positions = mt5.positions_get(symbol="USDJPY")
      #  if positions:
       #     for pos in positions:
        #        close_position(pos._asdict())
        #else:
         #   print("Không có vị thế nào để đóng cho USDCAD.")
        # Tự động bắt đầu bot
        start_bot()  
