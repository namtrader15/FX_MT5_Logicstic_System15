...
def calculate_volume_tool():
    ...
    # Lấy giá hiện tại của USDJPY từ MT5
    market_price = get_realtime_price_mt5(symbol="USDJPY")
    if market_price is None:
        return

    # Lấy ATR và tính Stop Loss từ MT5
    atr_symbol = "USDJPY"  # Thay đổi từ BTCUSDT thành USDJPY
    atr_short_stop_loss, atr_long_stop_loss = atr_stop_loss_finder(atr_symbol)

    # Tính volume cho lệnh Buy
    buy_volume = calculate_volume_based_on_risk("USDJPY", risk_amount, market_price, atr_long_stop_loss)
    if buy_volume is None:
        print("Không thể tính toán volume cho lệnh Buy.")
        return

    # Tính volume cho lệnh Sell
    sell_volume = calculate_volume_based_on_risk("USDJPY", risk_amount, market_price, atr_short_stop_loss)
