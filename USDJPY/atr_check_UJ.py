...
def atr_stop_loss_finder(symbol, length=14, multiplier=1.5, smoothing="RMA"): 
    # Lấy dữ liệu nến từ MetaTrader 5 (MT5) cho USDJPY, khung thời gian 1 giờ
    timeframes = {
    ...
    # Tính toán các mức Stop Loss với định dạng chính xác
    atr_short_stop_loss = highs[-1] + atr_value  # Không làm tròn
    atr_long_stop_loss = lows[-1] - atr_value   # Không làm tròn

    # In giá trị ATR và Stop Loss với 5 chữ số thập phân
    print(f"Giá trị ATR: {atr_value:.5f}")
    print(f"ATR Short Stop Loss: {atr_short_stop_loss:.5f}")
    print(f"ATR Long Stop Loss: {atr_long_stop_loss:.5f}")
    print(f"Giá cao nhất cây nến cuối: {highs[-1]:.5f}")
    print(f"Giá thấp nhất cây nến cuối: {lows[-1]:.5f}")
...
def main():
    ...
    symbol = "USDJPY"  # Thay thế thành cặp giao dịch USDJPY
