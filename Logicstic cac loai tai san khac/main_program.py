...
# Hàm chính
def main():
    while True:  # Vòng lặp để người dùng có thể chạy lại chương trình
        # Cho phép người dùng nhập mã tài sản từ bàn phím
        symbol = input("Nhập mã tài sản muốn phân tích (ví dụ: XAUUSD, EURUSD, BTCUSD): ").strip()
        lookback = 1500  # Số lượng nến lịch sử cần lấy

        print(f"\nĐang phân tích xu hướng cho mã tài sản: {symbol}")
        ...
        # Cho phép người dùng nhấn Enter để chạy lại hoặc nhập 'q' để thoát
        retry = input("\nNhấn Enter để phân tích mã tài sản khác, hoặc nhập 'q' để thoát: ").strip().lower()
        if retry == 'q':
