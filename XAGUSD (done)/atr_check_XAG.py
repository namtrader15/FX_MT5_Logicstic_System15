...
def atr_stop_loss_finder(symbol, length=14, multiplier=1.5, smoothing="RMA"): 
    # Lấy dữ liệu nến từ MetaTrader 5 (MT5) cho XAGUSD, khung thời gian 1 giờ
    timeframes = {
    ...
    atr_short_stop_loss = round(highs[-1] + atr_value, 3)  # ATR cho Short Stop Loss
    atr_long_stop_loss = round(lows[-1] - atr_value, 3)    # ATR cho Long Stop Loss

    # In ra các giá trị để anh kiểm tra
    print(f"Giá trị ATR: {atr_value:.2f}")
    print(f"ATR Short Stop Loss: {atr_short_stop_loss:.3f}")
    print(f"ATR Long Stop Loss: {atr_long_stop_loss:.3f}")
...
def main():
    ...
    symbol = "XAGUSD"  # Thay thế thành cặp giao dịch XAGUSD
