...
from atr_check_XAG import atr_stop_loss_finder  # Hàm tính ATR từ MetaTrader 5 (MT5)
import random
...
def get_realtime_price_mt5(symbol="XAGUSD"):
...
def place_order_mt5(client, order_type, symbol="XAGUSD", risk_amount=60, risk_reward_ratio=1.7):
    global last_order_status
    
    # Lấy giá mark hiện tại từ MT5 để đặt lệnh
    mark_price = get_realtime_price_mt5(symbol="XAGUSD")
    if mark_price is None:
        return

    # Sử dụng hàm ATR để lấy stop_loss dựa trên ATR từ MetaTrader 5 (MT5)
    atr_symbol = "XAGUSD"  # Đổi thành XAGUSD cho MT5
    ...
    stop_loss_price = float(f"{stop_loss_price:.3f}")

    # Tính khối lượng giao dịch dựa trên mức rủi ro và giá trị ATR
    volume = calculate_volume_based_on_risk("XAGUSD", risk_amount, mark_price, stop_loss_price)
    ...
    take_profit_price = float(f"{take_profit_price:.3f}")
    ...
    order = {
        ...
        "symbol": "XAGUSD",  # Thay đổi symbol thành XAGUUSD
        "volume": volume,
    ...
    if result is None:
        ...
        last_order_status = f"Đã {order_type} {volume} lots XAGUSD ở giá {mark_price:.3f} với Stop Loss: {stop_loss_price:.3f} và Take Profit: {take_profit_price:.3f}."
...
if __name__ == "__main__":
    ...
    if connect_mt5():
        ...
        place_order_mt5(None, "buy", "XAGUSD", risk_amount=60)  # Thay symbol thành "XAGUSD"
        mt5.shutdown()
    else:
