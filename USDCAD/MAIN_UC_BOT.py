...
from Entry_Super_UC import get_final_trend_USDCAD  # Hàm lấy xu hướng
from TPO_POC_UC import calculate_poc_value_USDCAD  # Hàm tính POC
from place_order_UC import place_order_mt5  # Hàm thực hiện lệnh giao dịch
...
def get_position_info():
    positions = mt5.positions_get(symbol="USDCAD")  # Thay BTCUSD bằng USDCAD
    if positions:
    ...
    return None
def get_positions_pnl_USDCAD(symbol="USDCAD"):
    """
    Hàm lấy tổng PNL của tất cả các lệnh mở trên USDCAD.
    """
    positions = mt5.positions_get(symbol=symbol)  # Lấy danh sách vị thế cho USDCAD
    if positions:
...
def update_trade_status():
    ...
    pnl_USDCAD = get_positions_pnl_USDCAD("USDCAD")  # Lấy PNL của USDCAD
    ...
    trade_status["pnl"] = pnl_USDCAD if pnl_USDCAD is not None else "N/A"  # Gán PNL của USDCAD
...
def get_trend():
    trend = get_final_trend_USDCAD()  # Không cần client Binance nữa
    print(f"Kết quả xu hướng USDCAD hiện tại: {trend}")
    return trend

# Kiểm tra giá POC và thực hiện lệnh
def check_poc_and_place_order(final_trend, symbol="USDCAD"):
    ...
    poc_value = calculate_poc_value_USDCAD()
    ...
    if price_difference_percent <= 0.025:
        ...
        print("[DEBUG] Không thực hiện lệnh vì chênh lệch vượt quá 0.025%.")
...
def close_position(position):
    ...
    """
    try:
        print("Bắt đầu xử lý đóng lệnh cho USDCAD...")
...
def buy_market():
    ...
    result = place_order_mt5(client, order_type="buy", symbol="USDCAD", risk_amount=RISK_AMOUNT)
...
def sell_market():
    client = mt5  # Tham số `client`, giả định nó là một phần của MetaTrader5
    result = place_order_mt5(client, order_type="sell", symbol="USDCAD", risk_amount=RISK_AMOUNT)
...
def status():
    update_trade_status()
    return jsonify({
        "current_price": get_realtime_price_mt5("USDCAD"),
        ...
        "pnl": trade_status["pnl"],  # PNL chỉ của USDCAD
        "tpo_poc_price": calculate_poc_value_USDCAD()  # Giá TPO/POC
    })
...
if __name__ == '__main__':
    if connect_mt5():
        print("Khởi động bot giao dịch USDCAD trên server Flask.")
